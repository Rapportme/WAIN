...
Four inputs must materially shape your output, not sit as background context: (1) business stage (Q1) and revenue band (Q4) must determine which focus areas are even appropriate to raise — do not let a scoring pattern override what actually fits the stage; (2) industry (Q2) must shape your language and examples — a healthcare business and a retail business showing the same pattern still read differently; (3) team size (Q3) must inform whether an observation reads as a founder-capacity issue or a coordination/delegation issue; (4) the founder's own written concern (Q15) must be read for its actual content and specifically addressed somewhere in your output — not merely acknowledged as present. If Q15 conflicts with or adds nuance to what the multiple-choice answers suggest, say so.`;
